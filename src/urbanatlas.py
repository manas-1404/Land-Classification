...
# Add the directories containing the utility files to the Python path
utils_path = r'C:\Users\lyq09mow\Code\Land-Classification\pysatml\pysatml\utils'
...
# Import the utility modules from the specific path
...
class UAShapeFile():
    ...
    def __init__(self, shapefile, prjfile=None, class_col='CLASS_2018', consolidate_classes=False, **kwargs):
        '''
        ...
        # Updated iteritems to items for Python 3
        ...
        print("Columns in the shapefile:", self._gdf.columns)
...
def load_shapefile(shapefile, class_col="ITEM"):
    # read in shapefile
    try:
        gdf = gpd.read_file(shapefile)  # Updated from from_file to read_file for Python 3
    except:
    ...
    targetcrs = {'ellps': 'WGS84', 'datum': 'WGS84', 'proj': 'longlat'}
...
#     # Step 3: Ensure class_col is not part of the index and is a column in the result
#     if class_col in gdf_sel.index.names:
#         print("Reset of column completed!\n")
#         gdf_sel.reset_index(inplace=True)

#     print("About to select polygon")
#     # select polygons to sample
#     select_polygons = gdf_sel.groupby(class_col)\
#                         .apply(lambda x: sample_polygons(x, n_samples=n_samples_per_class, max_samples=max_samples))
    

#     # Ensure class_col is not part of the index and is a column in the result
#     if class_col in select_polygons.index.names:
#         print("Reset of select polygonb completed!\n")
#         select_polygons.reset_index(inplace=True)
    
#     # Ensure class_col is a column
#     if class_col not in select_polygons.columns:
#         print("Class col is a coulmn!!\n")
#         select_polygons[class_col] = select_polygons.index.get_level_values(class_col)
#         select_polygons.reset_index(drop=True, inplace=True)
    
    
#     # make sure all polygons are ok? Step 3
...
#                 .apply(lambda x: sample_locations_from_polygon(x, sample_on_boundary = 'road' in x[class_col].iloc[0].lower() or 'railway' in x[class_col].iloc[0].lower()))
#     return locations

def generate_sampling_locations(gdf_sel, n_samples_per_class=N_SAMPLES_PER_CLASS,class_col="ITEM", max_samples=MAX_SAMPLES_PER_POLY):
    # Debug: Initial state
    print(f"Initial gdf_sel columns: {gdf_sel.columns}")
    print(f"Initial gdf_sel index names: {gdf_sel.index.names}")
    ...
    print("-"*50)
    print(f"After grouping, select_polygons columns: {select_polygons.columns}")
    print(f"After grouping, select_polygons index names: {select_polygons.index.names}")
        
    # Ensure class_col is a column without duplicating it
    print("-"*50)
    if class_col in select_polygons.columns:
        print(f"Before renaminng, select_polygons columns: {select_polygons.columns}")
        select_polygons.rename(columns={'CLASS_2018': 'ITEM_CLASS'},inplace=True)
        print(f"After renaming, select_polygons columns: {select_polygons.columns}")
    
    # make sure all polygons are ok
    print("-"*50)
    select_polygons['geometry'] = select_polygons['geometry'].apply(lambda p: p.buffer(0) if not p.is_valid else p)
    
    # Debug: After fixing geometries
    print(f"After fixing geometries, select_polygons head:\n{select_polygons.head()}")
    
    print("-"*50)
    # sample locations from each polygon
    locations = select_polygons.groupby("ITEM_CLASS")\
                .apply(lambda x: sample_locations_from_polygon(x,
                    sample_on_boundary = 'road' in x["ITEM_CLASS"].iloc[0].lower() or 'railway' in x["ITEM_CLASS"].iloc[0].lower()))
    
    # Debug: Final locations
    print(f"Final locations head:\n{locations.head()}")
...
def sample_polygons(df, n_samples=1000, max_samples=None):  
    ...
    if samples_per_poly.sum() > n_samples:
        ...
        # Updated ix to loc for Python 3
        ret_df = df.loc[ret_scaled.index]
...
def sample_locations_from_polygon(df, sample_on_boundary=False):
    ...
    if not sample_on_boundary:
        ...
        boundaries= [list(zip(p.exterior.coords.xy[0], p.exterior.coords.xy[1])) \
                     for p in polygons]  # Changed zip to list(zip)
